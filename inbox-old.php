...
$unreadComments = new MessageSet("New Comment Responses");
//create a message set for read comments
$readComments = new MessageSet("Read Comment Responses");
...
$unreadMessages = new MessageSet("New Direct Messages");
//create a set for red messages
$readMessages = new MessageSet("Read Direct Messages");
$inbox = $connection->query("SELECT * FROM messages M1 WHERE id=(SELECT MAX(id) FROM messages WHERE from_user=M1.from_user) AND to_user=$user_id");
...
class InboxMessage {
	...
	public function renderMessage(){
		$html = "<div class='comment' data-message-id='". $this->id ."'>";
		$html .= "<div class='comment-head'>";
		$html .= "<span>$this->username</span> posted at $this->date</div>";
		#$html .= "<a class='message-user-btn'>Message User</a> : ";
		#$html .= "<a href='inbox.php?id=$this->id'>View Entire Conversation</a>";
		#$html .= "</div>";
		$html .= "<p>". $this->comment ."</p>";
		$html .= "<div class='comment-actions'><a class='message-user-btn'>Message User</a> : <a href='inbox.php?id=$this->id#bottom'>View Entire Conversation</a> : <a href='functions/read-inbox-item.php?id=$this->id&context=message'>Mark as Read</a></div>";
		$html .= "</div>";
		echo $html;
	}
}
...
class MessageSet {
	public function __construct($newHeader){
	...
	public function renderSet(){
		if(count($this->messages) > 0){
			echo "<div class='inbox-header'>$this->header</div>";
			...
			<div id="inbox-body">
				...
				<?php
					...
					if( isset( $_SESSION['message_sent'] ) ){
						...
						echo "<div class='top-notification'>Comment has been moved to archive</div>";
